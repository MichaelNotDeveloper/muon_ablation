...
def make_algo_specs(
    lrs: List[float], include_gd=True, include_muon=True, include_adam=True
) -> List[AlgoSpec]:
    specs: List[AlgoSpec] = []
    if include_gd:
        for lr in lrs:
            specs.append(
                AlgoSpec(name=f"GD_lr_{lr:g}", opt_type="SGD", lr=lr, momentum=0.0)
            )
                ...
                for nest in [True]:
                    ...
                    for proj in ["exact", "ns"]:
                        if proj == "exact":
                            ...
                            muon_ns_steps = 5
                            name = (
                                f"Muon_ns{muon_ns_steps}_{nest_tag}_mom{mom:g}_lr{lr:g}"
                            )
                            specs.append(
                                AlgoSpec(
                                    name=name,
...
def run_sweep(args, device: str):
    ...
    for k_idx, kind in enumerate(args.kinds):
        ...
        for exp_id in range(args.num_experiments):
            ...
            W0 = torch.randn(args.d_in, args.d_out, device=device) / math.sqrt(
                args.d_in
            )

            results = compare_traces(
                loss_fn, [W0], algos, args.steps, sched_ctor=sched_ctor
            )
            ...
            finals = {
                name: float(tr["loss"][-1].item()) for name, tr in results_cpu.items()
            }
...
def run_plot(args):
    ...
    for p in sorted(os.listdir(root)):
        ...
        if not os.path.isdir(inst_dir):
            continue
        if os.path.isfile(os.path.join(inst_dir, "problem.pt")) and os.path.isdir(
            os.path.join(inst_dir, "experiments")
        ):
    ...
    for inst_dir in candidates:
        ...
        print(
            f"[PLOT] instance={os.path.basename(inst_dir)} exps={len(exp_files)} -> {plot_dir}"
        )
        ...
        if args.plot_separate:
            ...
            for exp_path in exp_files:
                ...
                kind = problem.get("kind", "unknown")
                title_prefix = (
                    f"kind={kind} | {os.path.basename(inst_dir)} | exp={exp_id:04d}"
                )
...
def _infer_available_algos(
    root: str, kinds: List[str], match: Optional[str], max_instances: Optional[int]
) -> List[str]:
...
def run_table(args):
    ...
    chosen_key_counts: Dict[
        str, Dict[str, Dict[str, int]]
    ] = {}  # kind->group->key->count
        ...
        for ef in exp_files:
            ...
            for group_name, pat, cands in groups:
                ...
                def final_loss(key: str) -> float:
                    ...
                    L = (
                        int(loss_trace.numel())
                        if torch.is_tensor(loss_trace)
                        else len(loss_trace)
                    )
                ...
                vals.setdefault(kind, {}).setdefault(group_name, {}).setdefault(
                    "t10", []
                ).append(v10)
                vals.setdefault(kind, {}).setdefault(group_name, {}).setdefault(
                    "t2", []
                ...
                chosen_key_counts.setdefault(kind, {}).setdefault(
                    group_name, {}
                ).setdefault(best_key, 0)
    ...
    for group_name, pat, _ in groups:
        ...
        for kind in args.kinds:
            ...
            print(
                f"{kind:18s} | {fmt(m10, se10, n10):22s} | {fmt(m2, se2, n2):22s} | {fmt(me, see, ne):22s}"
            )
...
def _best_group_value_by_time(
    results: Dict[str, Any], keys: List[str], idx: int
...
def run_table_ratios_same_dir(args):
    ...
    if not args.table_groups or len(args.table_groups) != 2:
        print(
            "[TABLE-RATIOS] ERROR: please pass exactly TWO groups via --table_groups."
        )
    ...
    for g in args.table_groups:
        ...
        if not keys:
            print(
                f"[TABLE-RATIOS] warning: group '{name}' pattern '{pat}' matched nothing"
            )
    ...
    print("\n" + "=" * 120)
    print(
        f"[TABLE-RATIOS] A vs B win-rate using best-by-time across LR (best loss in [0..t])"
    )
    print(f"[TABLE-RATIOS] A={nameA}  vs  B={nameB}")
    print(
        f"[TABLE-RATIOS] log_ratio = log(A+eps) - log(B+eps), eps={eps:.1e}, tol={tol:g}"
    )
    print("=" * 120)
    ...
    for kind in args.kinds:
        ...
        print(
            f"{kind:30s} | {fmt_cell(r10):{colw}s} | {fmt_cell(r2):{colw}s} | {fmt_cell(re):{colw}s}"
        )
...
def run_table_ratios(args):
    ...
    if unpairedA:
        print(
            "[TABLE-RATIOS] Unpaired instances from outdir A (after stripping instseed):"
        )
    ...
    if not paired:
        ...
        print(
            "  If your two outdirs were generated with different instance_dirname strings,"
        )
        print(
            "  you’ll need a more robust pairing key (e.g. match on problem.pt fields)."
        )
        return
    ...
    if not available_keysA:
        print(
            "[TABLE-RATIOS] could not infer available keys from outdir A (no experiments?)"
        )
        return
    if not available_keysB:
        print(
            "[TABLE-RATIOS] could not infer available keys from outdir B (no experiments?)"
        )
        return
    ...
    if not keysA:
        print(
            f"[TABLE-RATIOS] warning: group A '{nameA}' pattern '{patA}' matched nothing in outdir A"
        )
    if not keysB:
        print(
            f"[TABLE-RATIOS] warning: group B '{nameB}' pattern '{patB}' matched nothing in outdir B"
        )
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="substring filter on instance dir basename",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="if set, write plots here instead of inside each instance",
    )
    ...
    p.add_argument(
        "--no_show",
        action="store_true",
        help="do not display figures (recommended for batch); still saves PNGs",
    )
