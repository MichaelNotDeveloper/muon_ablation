...
def make_s(r, kind="flat", s_max=1.0, s_min=1e-2, alpha=1.0, device="cpu"):
    ...
    if kind == "flat_max":
        ...
        if kind == "gaussian":
            ...
            try:
                ...
                raise ValueError(
                    f"Could not parse k from kind='{kind}'. Expected 'gaussian_<k>' like gaussian_2.5"
                ) from e
        if k <= 0:
    ...
    # expected: "geometric_<q>"
        ...
        try:
            ...
            raise ValueError(
                f"Could not parse q from kind='{kind}'. Expected like 'geometric_0.93'."
            ) from e
        if not (0.0 < q < 1.0):
    ...
    # kind format: "spikes_N_pmin_pmax" e.g. "spikes_6_0.2_0.05"
        ...
        if not (0.0 <= pmin <= 1.0 and 0.0 <= pmax <= 1.0):
            raise ValueError(
                f"pmin,pmax must be in [0,1], got pmin={pmin}, pmax={pmax}"
            )
        if pmin + pmax > 1.0:
        ...
        if N == 2:
            if rem > 0:
                k = int(
                    torch.randint(low=0, high=rem + 1, size=(1,), device=device).item()
                )
                n_min += k
                n_max += rem - k
            s = torch.cat(
        ...
        if s.numel() != r:
            raise RuntimeError(
                f"Internal error: built s has length {s.numel()} != r={r}"
            )
...
def make_X_from_singular_values(n, d_in, s, device, dtype=torch.float32):
    ...
    r = s.numel()
    U, _ = torch.linalg.qr(
        torch.randn(n, r, device=device, dtype=dtype), mode="reduced"
    )
    V, _ = torch.linalg.qr(
        torch.randn(d_in, r, device=device, dtype=dtype), mode="reduced"
    )
    return (U * s) @ V.T
