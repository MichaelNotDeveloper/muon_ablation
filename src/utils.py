...
def run(
    loss_fn, params0: list[torch.tensor], opt_ctor, steps: int, sched_ctor=None
) -> list[float]:
    """
...
def compare_traces(
    loss_fn, params0, algos: dict, steps: int, sched_ctor=None, **kwargs
):
    return {
        name: run_trace(loss_fn, params0, opt_ctor, steps, sched_ctor, **kwargs)
        for name, opt_ctor in algos.items()
    }
